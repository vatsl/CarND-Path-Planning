...
int main() {
  ...
  double ref_vel = 0.4; //mph

  // Keep a count of lane changes in the last 5 cycles
  int num_lane_shifts = 1;

  h.onMessage([&ref_vel, &map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy, &lane, &num_lane_shifts](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode) {
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
		...
		if(prev_size < 2){
		    ...
		    // redfine reference states as previous path end points
		...
		// Calculate how to break up the spline points so that we travel at out desired reference velocity
